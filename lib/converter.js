...
var _phoneLib             = require('phone');
var normalizePhone        = _phoneLib.phone || _phoneLib.default || _phoneLib;
var logger                = require('./debug-log');
var ProgressBar           = require('ts-progress');
var prettyoutput          = require('prettyoutput');
var { parseAttributedBody } = require('./parse-attributed-body');
...
class Converter {
  ...
  uniqueId(message){
    var info = [message.sender, message.receiver, message.date, message.message_text, message.service];
  ...
  internalIdentifier(row) {
    ...
    var info = [row.address, row.date, row.message_text, row.service];
    return Crypto.createHash('sha1').update(JSON.stringify(info)).digest('hex');
  }
  ...
  mapAttachment(id, row) {
    if ((row.attachment && row.attachment.length > 0) || row.attachment_mime_type) {
      // has an attachment
  ...
  buildContentSegments(message, row) {
    ...
    if (row.associated_message_guid) {
      ...
      var parts = message.message_text.split("\uFFFC");
  ...
  formatAddress(value) {
    ...
    var result = normalizePhone(value);
    ...
    if (otherAddresses.length === 1 && !!otherAddresses[0].address) { // nailed it
      ...
      if (otherAddresses.length > 1) {
        ...
        if (filteredAddresses.length === 1) {
          ...
          return _.sortBy(otherAddresses, (i => i.count))[0].address;
        }
  ...
  buildPayload() {
    ...
    for (var i = ordered.length; i >= 0; i--) {
      ...
      if (row) {
        ...
        // Resolve message text: prefer the plain text column; fall back to
        // parsing attributedBody (NSAttributedString blob, common in iOS 16+)
        var resolvedText = row.message_text;
        if (!resolvedText && row.attributedBody) {
          resolvedText = parseAttributedBody(row.attributedBody);
        }
