(function() {
  ...
  function addQueryOptions(query, options) {
    ...
    if (options.search) {
      // Also match messages stored only in attributedBody (iOS 16+ rich text).
      // attributedBody is a binary blob but CAST … AS TEXT exposes the plaintext
      // fragments that SQLite can substring-match on without full deserialisation.
      additions += ` AND (text LIKE "%${options.search}%" OR CAST(attributedBody AS TEXT) LIKE "%${options.search}%")`;
    }
